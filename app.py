...
def paymentsPix():
    ...
    if "value" not in data:
        return jsonify({"message": "numero invalido"}), 400
    ...
    return jsonify({"message": "pagamento criado ", "pagamento": new_payment.to_dict()})
...
def paymentsPixConfirmation():
    ...
    if not payment or payment.paid:
        return jsonify({"message": "valor invalido "}), 404
...
def paymentsPixPage(payment_id):
...
def handle_connect():
    print("client disconnect to the server")  
